...
COUNT=0
while true; do
    FILELIST=$( ./split_file.sh $WORKDIR/filelist.txt 20 $COUNT )
...
#PBS -o $WORKDIR/output.$COUNT.txt
#PBS -e $WORKDIR/error.$COUNT.txt
#PBS -j oe
...
$FILELIST
EOINPUT
echo "input list is \$INPUTLIST"
python2.6 shyft_fwlite.py --inputListFile=\$INPUTLIST --sampleName=$4 $OURDATA --lepType=0 --outname=$WORKDIR/$COUNT
